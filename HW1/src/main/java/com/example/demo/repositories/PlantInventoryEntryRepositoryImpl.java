	...
	public List<PlantsWithRentalsAndRepairs> findRentalsAndRepairs(int year) {
		return entityManager.createQuery(
				"select new com.example.demo.models.PlantsWithRentalsAndRepairs(pie, count(purchaseOrders), count(tasks)) " +
						...
						"left join pie.purchaseOrders purchaseOrders on purchaseOrders.status = com.example.demo.models.POStatus.CLOSED " +

						// TODO need to take year into account instead
						"and purchaseOrders.status = com.example.demo.models.POStatus.CLOSED " +
						// " and purchaseOrders.issueDate.year = ?1 " +
