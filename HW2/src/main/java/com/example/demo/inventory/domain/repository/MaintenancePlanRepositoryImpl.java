...
public class
MaintenancePlanRepositoryImpl implements MaintenancePlanRepositoryCustomQueries {
...
/*entityManager.createQuery("select new com.example.demo.utils.Pair(mp.yearOfAction, sum(mt.price)) from MaintenancePlan mp JOIN mp.tasks mt  " +
				"" + "where mp.yearOfAction >= ?1 and mp.yearOfAction <= ?2 and mt.typeOfWork = com.example.demo.inventory.domain.model.TypeOfWork.CORRECTIVE group by mp.yearOfAction ORDER BY mp.yearOfAction")
				.setParameter(1, startYear)
				.setParameter(2, endYear)
				.getResultList();*/
